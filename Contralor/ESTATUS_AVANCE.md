...
- **Buzón de correo** (platform-admin): API devuelve `{ entries, tableMissing? }`; página con actualizar, filtros (tipo, correo), vista previa, aviso si falta tabla con instrucción `npm run db:migrate`.
- **Validación:** `scripts/check-db-tables.cjs` ahora verifica también la tabla `email_log`. Comando: `npm run db:check`.
- **Resumen:** Base de datos lista para backup. Contralor puede ejecutar backup cuando Henry autorice.
...
| 06 Mar 2026 | **✅ DATABASE informa al Contralor:** Migración 108_email_log aplicada en BD (vía Docker). Tabla email_log creada (id, to_email, subject, email_type, body_preview, success, created_at). Índices en created_at, to_email, email_type. Para futuras migraciones: npm run db:migrate (requiere DATABASE_URL en .env) o manualmente: docker compose exec -T postgres psql -U postgres -d assembly -f - \< sql_snippets/108_email_log.sql. Buzón correo (dashboard Henry) puede usar la tabla. | Database |
